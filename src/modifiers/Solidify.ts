...
class Solidify extends Modifier {
	private thickness: number | Array<number>
	...
	public apply(buffer: Float32Array) {
		return Solidify.solidify(buffer, this.thickness)
	}

	/**
	 * @link https://gist.github.com/kekscom/4194148
	 */
	public static solidify(buffer: Float32Array, thickness: number | Array<number>): Float32Array {
		...
		for (let i = 0, len = buffer.length - 2; i < len; i += 2) {
			...
			const off = getOffsets(buffer[i], buffer[i + 1], buffer[i + 2], buffer[i + 3], thickness[i] || thickness[0])
			const off2 = getOffsets(buffer[i], buffer[i + 1], buffer[i + 2], buffer[i + 3], thickness[i + 1] || thickness[0])
